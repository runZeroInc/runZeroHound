...
// Match_IPv6 matches a valid IPv6 address.
...
// Match_IPv4 matches a valid IPv4 address.
...
// IPv4_Masks maps host count to CIDR prefix length.
...
// IPv4_Mask_Sizes lists power-of-two block sizes in descending order.
...
// IPv4_to_UInt converts an IPv4 address string to a uint32.
...
// UInt_to_IPv4 converts a uint32 to an IPv4 address string.
...
// IPv4Range2CIDRs returns the minimal list of CIDRs covering [s_ip, e_ip].
...
// IPv4UIntRange2CIDRs returns the minimal CIDR list for a uint32 range.
func IPv4UIntRange2CIDRs(s_i uint32, e_i uint32) []string {
	...
	for i := range IPv4_Mask_Sizes {
		...
		cidrs = append(cidrs, fmt.Sprintf("%s/%d", UInt_to_IPv4(s_i), IPv4_Masks[mask_size]))
		s_i = s_i + mask_size
		for _, c := range IPv4UIntRange2CIDRs(s_i, e_i) {
...
// AddressesFromCIDR enumerates all host addresses in a CIDR and sends them to
// the output channel.
func AddressesFromCIDR(cidr string, o chan<- string) {
	...
	mask_ones, mask_total := ipNet.Mask.Size()
	net_size := uint32(math.Pow(2, float64(mask_total-mask_ones)))
	end_base := net_base + net_size
	for cur_addr := net_base; cur_addr < end_base; cur_addr++ {
